...
# Read source files — supply the Gemini source HTML as a command-line argument
# Usage: python merge_gemini.py /path/to/gemini-source.txt
HERE = os.path.dirname(os.path.abspath(__file__))
if len(sys.argv) < 2:
    print("Usage: python merge_gemini.py <gemini-source-file>")
    sys.exit(1)
gemini = open(sys.argv[1], 'r', encoding='utf-8').read()
app = open(os.path.join(HERE, 'ui_parts', 'app.html'), 'r', encoding='utf-8').read()
...
# Write the parts
open(os.path.join(HERE, 'ui_parts', 'head.html'), 'w', encoding='utf-8').write(head)
open(os.path.join(HERE, 'ui_parts', 'styles_and_scene.html'), 'w', encoding='utf-8').write(scene)
