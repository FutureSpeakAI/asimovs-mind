...
def fnv1a_hash(data: str) -> str:
    """Generate an FNV-1a hash of the input string, returned as hex."""
    h = FNV_OFFSET_BASIS
...
def make_placeholder(category: PIICategory, original: str) -> str:
    """Create a deterministic placeholder like [SSN:a3f8b2c1e4d5f607]."""
...
WATCHLIST_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".name_watchlist.json")

_DEFAULT_WATCHLIST = [
    # Add your own names here, or use the watchlist_add() function.
...
# Bank account / routing numbers: "account" or "routing" followed by digits
_BANK_PATTERN = re.compile(
    r'(?i)\b(?:account|acct|routing|aba)[#:\s-]*(\d{6,17})\b'
)

# Phone numbers: US formats
_PHONE_PATTERN = re.compile(
    r'(?<!\d)'                       # not preceded by digit
    r'(\(?\d{3}\)?[-.\s]\d{3}[-.\s]\d{4})'  # (512) 814-7609 or 512-814-7609
...
def _find_bank_accounts(text: str) -> list[tuple[int, int, str]]:
    ...
    for m in _BANK_PATTERN.finditer(text):
        # Capture the full match including the label
...
def scan_text(
    ...
    """
    Scan text for PII. Returns a list of PIIMatch objects sorted by position.
    
    Args:
        text: The text to scan.
        categories: Which PII categories to scan for. None = all.
        allowlist: Values to skip. None = load from .allowlist.json.
    """
...
def scrub_text(
    ...
    """
    Replace all detected PII with hashed placeholders.
    
    Returns:
        A tuple of (scrubbed_text, matches) so the caller can unscrub later.
    """
...
def unscrub_text(scrubbed: str, matches: list[PIIMatch]) -> str:
    """
    Restore original PII from a scrubbed string using the match list.
    """
    result = scrubbed
    for m in matches:
