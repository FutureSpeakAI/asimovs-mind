...
# Staleness window (days) for demotion
STALENESS_DAYS = 14
...
@dataclass
class MemoryEntry:
    """A single memory entry stored in a tier."""
    ...
    sessions: int = 1  # cross-session count
...
@dataclass
class Episode:
    """An episodic memory — an append-only session recording."""
...
def _tokenize(text: str) -> set[str]:
    """Lowercase word-set with stop-word filtering."""
...
class CognitiveMemory:
    """Three-tier cognitive memory with deduplication and consolidation."""
    ...
    def add(
        ...
        """Add a new memory entry, or reinforce an existing near-duplicate."""
        if isinstance(tier, str):
            tier = MemoryTier(tier)

        # Check for near-duplicate in same tier
    ...
    def find(self, query: str, tier: Optional[MemoryTier] = None) -> list[MemoryEntry]:
        """Find entries by Jaccard word-overlap, optionally filtered by tier.

        Returns entries sorted by descending similarity (> 0 overlap only).
        """
    ...
    def jaccard_similarity(a: str, b: str) -> float:
        """Word-set Jaccard similarity with stop-word filtering."""
        set_a = _tokenize(a)
        set_b = _tokenize(b)
    ...
    def deduplicate(self, tier: MemoryTier) -> int:
        """Merge entries with ≥80% Jaccard similarity within a tier.
    ...
    def compute_promotion_score(entry: MemoryEntry) -> float:
        """Weighted promotion score for a medium-term entry.

        Components:
            Frequency:      min(occurrences * 2, 20)
            Cross-session:  min(sessions * 2, 10)
            Time-span:      +5 if observed ≥7 days
            Confidence:     +3 if confidence ≥ 0.9
            Staleness:      -5 if >14 days since last_seen
        """
        score = 0.0
        ...
        # Cross-session (max 10)
        score += min(entry.sessions * 2, 10)

        # Time-span persistence
        days_observed = _days_since(entry.first_seen)
        if days_observed >= 7:
            score += 5
        ...
        # Staleness penalty
        days_stale = _days_since(entry.last_seen)
    ...
    def promote(self) -> list[MemoryEntry]:
        """Promote qualified medium-term entries to long-term.

        Requires: composite score ≥ 10 AND occurrences ≥ 3.
    ...
    def demote(self) -> list[MemoryEntry]:
        """Demote stale long-term entries back to medium-term.

        An entry is stale if >14 days since last_seen.
        Returns list of demoted entries.
        """
    ...
    # -- Consolidation (sleep-like cycle) ------------------------------------

    def consolidate(self) -> dict[str, Any]:
        """Run the full sleep-like consolidation cycle.

        Steps: deduplicate all tiers → promote → demote → record timestamp.
    ...
    def record_episode(
        ...
        """Record an episodic memory (session recording)."""
        episode = Episode(
            summary=summary,
            topics=topics or [],
            emotional_tone=tone,
    ...
    def get_stats(self) -> dict[str, Any]:
        """Return counts per tier, total entries, and last consolidation time."""
        return {
